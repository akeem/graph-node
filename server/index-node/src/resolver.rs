use graphql_parser::{query as q, query::Name, schema as s, schema::ObjectType, query::Number};
...
impl IndexingStatus {
    ...
    fn u64_from_value(value: &q::Value, key: &'static str,) -> Result<u64, Error> {
        value.get_required::<q::Value>(key.as_ref())
            .and_then(|value| match value {
...
impl From<IndexingStatus> for q::Value {
    fn from(status: IndexingStatus) -> Self {
        object_value(vec![
            ...
            ("entity_count", q::Value::String(format!("{}", status.entity_count))),
            ("query_price", q::Value::Int(Number::from(status.query_price as i32))),
        ])
    }
}
